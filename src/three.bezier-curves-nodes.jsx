...
import { createContext, useMemo, useRef, useState, useContext, useLayoutEffect, forwardRef, useEffect, createRef } from 'react'
    ...
    <context.Provider value={set}>
      <group ref={group}>
        {lines.map((line, index) => (
          <group key={`g1-${index}`}>
            <QuadraticBezierLine key={`L1-${index}`} {...line} color="white" dashed dashScale={50} gapSize={20} />
            <QuadraticBezierLine key={`L2-${index}`} {...line} color="white" lineWidth={0.5} transparent opacity={0.1} />
          </group>
        ))}
      </group>
      {children}
      {lines.map(({ start, end }, index) => (
        <group key={`g2-${index}`} position-z={1}>
          <Circle key={`C1-${index}`} position={start} />
          <Circle key={`C2-${index}`} position={end} />
        </group>
      ))}
    </context.Provider>
  )
